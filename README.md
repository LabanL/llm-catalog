...
实时对比 16 家厂商、51 款主流大模型的 API 定价、上下文窗口、多模态能力与综合评分。

本仓库包含三份数据清单：
- **[`index.html`](./index.html)** — 文本大模型（对话/推理/代码）：16 家厂商，51 款模型
...
- **2026-09-22**：
  - 小米：新增 **MiMo V2.6 Pro / V2.6 Flash**（9/21 权重开源、9/22 正式发布，MIT 协议；Pro 1.02T MoE / 42B 激活、AA 智能指数 46 超过 Kimi K3 与 Qwen3.8 Max，为当前最强开源模型；Flash 309B MoE / 15B 激活、256 路由专家；两者均为原生全模态（文本 / 图像 / 音频 / 视频输入），1M 上下文，API 定价沿用 V2.5 系列——Pro ¥3 / ¥0.025 缓存 / ¥6，Flash ¥1 / ¥0.02 / ¥2；Pro 另提供 UltraSpeed 高速档，最高 20 倍推理速度；开源含权重、7k+ RL 任务环境与端到端 RL 训练框架）；V2.5 系列暂保留
  - xAI：新增 **Grok 4.7**（9/21 发布，主打编码与知识工作，参数 2.1T 较 4.6 增长 40%，500K 上下文，$2 / $6 定价与上代持平，另有输出速度两倍、价格两倍的高速变体，已在 Cursor / Grok Build / Grok API 上线）；Grok 4.6 保留；评分待 AA 下一版指数收录（发布晚于 v4.3 快照 9/14）
